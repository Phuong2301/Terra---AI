...
class RiskScoringEngine {
  static RiskScoreResult evaluate(Map<String, dynamic> payload) {
    ...
    final aiAdj = _aiAdjustment(
      ...
      income: income,
      debt: debt,
      province: (payload['province'] ?? '').toString(),
    ...
    final cat = _category(finalScore);
    final rec = _recommendation(cat);
    ...
    final without = _termsFromScore(base.clamp(0, 100));
    final withFpo = _termsFromScore(finalScore);
  ...
  static String _category(int score) {
    if (score >= 75) return 'Low Risk';
    if (score >= 50) return 'Medium Risk';
    return 'High Risk';
  }

  static String _recommendation(String category) {
    switch (category) {
      case 'Low Risk':
        return 'Recommend approval with standard terms. Monitor seasonal cash flow.';
      case 'Medium Risk':
        return 'Recommend conditional approval: smaller limit and stronger repayment schedule.';
      default:
        return 'Recommend caution: verify income stability, reduce exposure, consider guarantees.';
    }
  }

  static LoanTerms _termsFromScore(int score) {
    final maxAmount = _lerp(300, 2000, score / 100.0); 
    final interest = _lerp(28, 14, score / 100.0);     
    final tenure = (6 + (score / 100.0 * 12)).round();
    final repay = score >= 75 ? 'Monthly' : (score >= 50 ? 'Bi-weekly' : 'Weekly');
  ...
  static String _buildReasoning({
    ...
    final parts = <String>[
      ...
      'AI adjustment = ${aiAdj >= 0 ? '+' : ''}$aiAdj based on patterns from similar farmer profiles in ${_loc(province, district)} and your inputs (Repayment: $repaymentHistory, DTI: $dtiStr).',
    ];

    if (isFpo) {
      parts.add('FPO boost = +$fpoBoost due to collective track record effect in the model.');
    } else {
      parts.add('No FPO boost applied (non-member).');
    }

    parts.add('Final score = $finalScore (capped at 100).');
    return parts.join(' ');
  }

  static String _loc(String province, String district) {
    final p = province.trim();
    final d = district.trim();
    if (p.isEmpty && d.isEmpty) return 'your region';
